...
export class DataService {
  ...
  sortPriceAsc() {
    const courses = this.courses.sort(
      (a, b) => parseInt(a.actualPrice) - parseInt(b.actualPrice)
    );
  ...
  sortPriceDesc() {
    const courses = this.courses.sort(
      (a, b) => parseInt(b.actualPrice) - parseInt(a.actualPrice)
    );
    this.paginateCoursesMethod(courses);
    this.currentPage = 1;
    return this.getCourse(this.currentPage);
